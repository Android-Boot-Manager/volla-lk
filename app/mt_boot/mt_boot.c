...
{	
	int calidata_len =347; // hall calidata addr 341,length :6 
	//int i =0;
	int ret = 0 ;
...
/* begin, prize-lifenfen-20181120, add for hardware info, ddr, etc. */
	...
	hardinfo_pbuf = g_boot_arg->prize_ddr_hardinfo;
	dprintf(0, "hardinfo_pbuf = %s\n", hardinfo_pbuf);
	snprintf(tmpbuf, TMPBUF_SIZE, "prize_ddr_hardinfo=%s", hardinfo_pbuf);
	cmdline_append(tmpbuf);
#endif
/* end */
	/*prize  add  for hall  calibration by zhuzhengjiang    20190221-begin*/
	int len=240;
	char hall_calidata_buf[261] = {0};
	//len =mboot_common_get_hall_calidata(hall_calidata_buf);
	mboot_recovery_load_raw_part_offset("proinfo", hall_calidata_buf,1536*1024,260);
	for(;len<250;len++)
	{
	    //pal_log_err("tangan prize proinfo buffer=%d\n",buf[j]);
        pal_log_err("prize proinfo hall_calidata_buf[%d]=%d\n",len,hall_calidata_buf[len]);
	}
	snprintf(cmdline_tmpbuf, CMDLINE_HALL_CALIDATA_SIZE, "hall_up_cali_data=%d%d%d", hall_calidata_buf[244]-'0',hall_calidata_buf[245]-'0',hall_calidata_buf[246]-'0');
	cmdline_append(cmdline_tmpbuf);
	   
	snprintf(cmdline_tmpbuf, CMDLINE_HALL_CALIDATA_SIZE, "hall_down_cali_data=%d%d%d", hall_calidata_buf[247]-'0',hall_calidata_buf[248]-'0',hall_calidata_buf[249]-'0');
	cmdline_append(cmdline_tmpbuf);
	/*prize  add  for hall  calibration by zhuzhengjiang    20190221-end*/
