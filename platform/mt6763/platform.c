		...
		for (tags = (void *)BOOT_ARGUMENT_LOCATION; tags->hdr.size; tags = boot_tag_next(tags)) {
			switch (tags->hdr.tag) {
				...
				case BOOT_TAG_SOC_ID:
					...
					break;
/* begin, prize-tangcong-20200819, add for hardware info, ddr, etc. */
				case BOOT_TAG_LPDDR_EMCP_INFO:
					g_boot_arg->emmc_lpddr_used_index=tags->u.lpddr_emcp_info.pl_lpddr_emcp_use_index;
					memcpy(&(g_boot_arg->prize_ddr_hardinfo), &(tags->u.lpddr_emcp_info.pl_lpddr_emcp_hardinfo), sizeof(g_boot_arg->prize_ddr_hardinfo));
					break;
/* end */
				default:
					break;
